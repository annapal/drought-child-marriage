...
plot_te_time <- function() {

  # Load the results data
  results <- read_excel("results/tes_by_yr.xlsx")
  results$country <- countrycode(results$iso, "iso3c", "un.name.en")
  results$drought_yr <- factor(results$drought_yr, levels = c(3,2,1))
  
  # Plot
  p <- ggplot(results, aes(x = estimate, y = drought_yr)) +
